...
async function setFormDataJob(asset, json, token, clientId) {
	let body = {
		'assetID': asset.assetID,
		'jsonFormFieldsData':json
	}

	let resp = await fetch(REST_API + 'operation/setformdata', {
		method: 'POST', 
		headers: {
			'Authorization':`Bearer ${token}`, 
			'X-API-KEY':clientId,
...
(async () => {
	...
	let json = JSON.parse(fs.readFileSync('./get_form.json', 'utf8'));

	let job = await setFormDataJob(uploadedAsset, json, accessToken, CLIENT_ID);
	...
	*/
	...
	//console.log(result);
	await downloadFile(result.asset.downloadUri, 'set_form.pdf');
