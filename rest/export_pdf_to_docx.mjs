...
async function getExportJob(asset, targetFormat, ocrLang='en-US', clientId, token) {
...
let input = '../source_pdfs/chinese_traditional.pdf';
...
let job = await getExportJob(assetId, 'docx', 'zh-Hant', CLIENT_ID, token);
let result = await pollJob(job, CLIENT_ID, token);

await downloadFile(result.asset.downloadUri, './export_to_docx.docx');
console.log('Done');